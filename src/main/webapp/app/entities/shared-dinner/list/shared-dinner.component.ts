...
export class SharedDinnerComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.sharedDinnerService
      ...
      .subscribe(
        (res: HttpResponse<ISharedDinner[]>) => {
  ...
  protected onSuccess(data: ISharedDinner[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/shared-dinner'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
